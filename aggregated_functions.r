...
# Function returning an empty model
    ...
    if (error_code == 0) {
      ...
      if (min(bb$ndays.x == bb$ndays.y) == 0) {
        ...
        if (!is.null(CB)) {
          ...
          X[(dat[,name_period] %in% to_exclude) , name_exposure] <- NA
        }
      ...
      if (ncol(X) >= length(y)) {
        ...
        if ((mleBHHH$code %in% c(1,2,8))) { # converged
          ...
          converged <- ((mle$code %in% c(1,2,8)) & is.positive.semi.definite(Vdisp) )
